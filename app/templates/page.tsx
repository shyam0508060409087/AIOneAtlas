      ...
      <div style={{ background: "var(--bg0)", overflowY: "auto" }}>
        <div className="px-8 pt-7 pb-5" style={{ borderBottom: "1px solid var(--b0)" }}>
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
            ...
            <p className="text-sm mb-5" style={{ color: "var(--t3)" }}>Start with a battle-tested template. Customize with AI in seconds.</p>
            <div className="flex items-center gap-2.5 max-w-[540px] px-3.5 py-2.5 rounded-xl" style={{ background: "var(--bg2)", border: "1px solid var(--b1)" }}>
              <span className="text-[15px]" style={{ color: "var(--t4)" }}>🔍</span>
              <input className="flex-1 bg-transparent outline-none text-sm" style={{ color: "var(--t1)" }} placeholder="Search templates..." value={search} onChange={(e) => setSearch(e.target.value)} />
            </div>
          </motion.div>
        </div>

        <div className="flex items-center justify-between px-8 py-3.5 gap-3 overflow-x-auto" style={{ borderBottom: "1px solid var(--b0)" }}>
          <div className="flex gap-1.5 flex-shrink-0">
            {TEMPLATE_CATEGORIES.map((cat) => (
              <motion.button key={cat} className={`px-3.5 py-1.5 rounded-full text-xs font-medium whitespace-nowrap transition-all ${activeCategory === cat ? "text-accent-light" : "text-text-3 hover:text-text-1"}`} style={{ background: activeCategory === cat ? "rgba(99,91,255,0.1)" : "var(--bg2)", border: `1px solid ${activeCategory === cat ? "rgba(99,91,255,0.3)" : "var(--b1)"}` }} onClick={() => { setActiveCategory(cat); router.push(`/templates?cat=${cat}`); }} whileTap={{ scale: 0.97 }}>{cat}</motion.button>
            ))}
          </div>
        </div>
          ...
          <div className="flex gap-3 overflow-x-auto pb-1">
            {FEATURED.map((t, i) => (
              <motion.div key={t.id} className={`flex-shrink-0 w-[250px] rounded-xl p-5 cursor-pointer transition-all ${FEATURED_STYLES[i]}`} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.07 }} whileHover={{ y: -3 }} onClick={() => setPreviewTemplate(t)}>
                ...
                <p className="text-[9px] font-bold tracking-[0.08em] uppercase mb-1.5" style={{ color: "var(--t3)" }}>{t.featuredLabel ?? "Featured"}</p>
          ...
          <AnimatePresence mode="wait">
            {filtered.length > 0 ? (
              ...
              <motion.div key="empty" className="flex flex-col items-center justify-center py-24 text-center" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
                ...
                <p className="text-sm max-w-[300px] leading-relaxed mb-5" style={{ color: "var(--t3)" }}>Try a different search term or browse all categories.</p>
              </motion.div>
            )}
          </AnimatePresence>
        </div>
